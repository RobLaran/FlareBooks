<div class="edit-book-page">
    <form action="<?= routeTo('/books/update/' . $book['ISBN']) ?>" method="POST" class="form-container" id="update-book-form">
        <div class="image-preview">
            ...
            <?php else: ?>
                <img src="<?= getFile('public/img/' . $book['image']) ?>" alt="Image Preview in File" id="image-preview" data-default="<?= getFile('/public/img/image-preview.jpg') ?>">
            <?php endif; ?>
        ...
        <div class="status-field input-container">
            ...
            <select name="status" id="status-selection">
                <option value="Available" class="online" <?= $book['status'] == 'Available' ? 'selected' : '' ?> >Available</option>
                <option value="Unavailable" class="offline" <?= $book['status'] == 'Unavailable' ? 'selected' : '' ?> >Unavailable</option>
            </select>
        </div>
            ...
            <select name="genre" id="genre-selection">
                <?php if(count($genres) > 0): ?>
                    <?php foreach($genres as $genre): ?>
                        <option value="<?= $genre['id'] ?>" <?= $book['genre_id'] == $genre['id'] ? 'selected' : '' ?> ><?= $genre['genre'] ?></option>
                    <?php endforeach; ?>
                <?php endif; ?>
            </select>
        </div>
        <div class="button-container">
            ...
            <button class="button default">Update</button>
